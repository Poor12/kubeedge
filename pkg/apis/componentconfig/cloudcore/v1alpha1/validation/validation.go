...
func ValidateModuleCloudStream(d v1alpha1.CloudStream) field.ErrorList {
	...
	if !utilvalidation.FileIsExist(d.TLSTunnelPrivateKeyFile) {
		klog.Infof("TLSTunnelPrivateKeyFile does not exist in %s, will load from secret", d.TLSTunnelPrivateKeyFile)
	}
	if !utilvalidation.FileIsExist(d.TLSTunnelCertFile) {
		klog.Infof("TLSTunnelCertFile does not exist in %s, will load from secret", d.TLSTunnelCertFile)
	}
	if !utilvalidation.FileIsExist(d.TLSTunnelCAFile) {
		klog.Infof("TLSTunnelCAFile does not exist in %s, will load from secret", d.TLSTunnelCAFile)
	}
